...
const imageHtml = images.map(img => {
    // Fix the path to use forward slashes or relative paths if needed
    const imgSrc = `static/Data/${img.replace(/\\/g, '/')}`; // Convert backslashes to forward slashes for URL compatibility
    return `<img src="${imgSrc}" alt="Similar image" style="width:100px; margin: 10px;">`;
}).join('');
